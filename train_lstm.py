"""
train_lstm_stream_allsubjects.py (FINAL CORRECTED VERSION)
Memory-safe LSTM training across all subjects (streams CSV rows) 
with crucial RAW DATA SCALING applied.
"""
...
SAVE_DIR = r"C:\Users\linah\Desktop\Final Year Project Linah\results\lstm_allsubjects"
    ...
    while True:
        with open(path, "r") as fh:
            for line in fh:
                ...
                if len(parts) < 5121:
                    continue
                # NOTE: The DeprecationWarning here is minor and harmless; it's due to np.fromstring.
...
# -----------------------------
# Streaming Sequence for Keras (UPDATED)
# -----------------------------
class StreamSequence(Sequence):
    def __init__(self, file_list, batch_size=32, subject_mode="train"):
        self.files = list(file_list)
        if not self.files:
            raise ValueError("No files provided to StreamSequence")
...
# -----------------------------
# Model builder: LSTM (UNCHANGED)
# -----------------------------
def build_lstm_model():
    model = Sequential([
        # input_shape should match (timesteps, features) which is (512, 10)
        LSTM(128, return_sequences=True, input_shape=(512,10)),
        Dropout(0.3),
        LSTM(64),
        Dropout(0.3),
...
# -----------------------------
# Plot helpers (FIXED: ensures all functions are present)
# -----------------------------
...
def main():
    ...
    train_seq = StreamSequence(train_files, batch_size=BATCH_SIZE, subject_mode="train")
    val_seq   = StreamSequence(test_files,  batch_size=BATCH_SIZE, subject_mode="test")

    model = build_lstm_model()
    model.summary()

    # callbacks
    best_path = os.path.join(SAVE_DIR, "lstm_best.keras")
    callbacks = [
    ...
    # save final model
    model.save(os.path.join(SAVE_DIR, "lstm_final.keras"))
    plot_history(history, "lstm") # <--- Now correctly defined

    # save architecture diagram
    try:
        plot_model(model, to_file=os.path.join(SAVE_DIR, "figs", "lstm_architecture.png"), show_shapes=True)
    except Exception as e:
        print("plot_model failed:", e)

    # quick evaluation: sample several batches from test files
    y_true, y_pred = [], []
    ...
    plot_confusion(y_true, y_pred, "lstm_sampled_eval")

    # summary CSV
    acc = accuracy_score(y_true, y_pred)
    pd = __import__("pandas")
    pd.DataFrame([{"model":"LSTM","accuracy":float(acc)}]).to_csv(os.path.join(SAVE_DIR,"summary_lstm.csv"), index=False)
    print("[DONE] LSTM training + sampled evaluation complete. Results in", SAVE_DIR)
